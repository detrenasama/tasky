...
- `main.go` — корневая BubbleTea-модель: переключение экранов (`tasks`/`projects`), тик 1с (обновление таймеров и недельной суммы), `tea.WindowSizeMsg`, выход по `q`/`ctrl+c`; здесь же общие стили lipgloss
- `tasks_screen.go` — главный экран: один список (`bubbles/list`) с раскрывающимися подзадачами (`enter`/`→` — раскрыть/свернуть, `▸`/`▾`, подзадачи с префиксом `├`), справа — недельная панель; `n` — новая задача, `a` — подзадача в выбранную задачу (или в родителя выбранной подзадачи), `d` — удаление (подтверждение y/n, каскад по БД), `ctrl+l` — старт/пауза подзадачи, `[`/`]` — смена проекта, `p` — проекты
